...
> 更新时间：2026-08-23
>
> 当前阶段：P0/P0.1 已完成；P1 本地学习库与手动 GitHub 同步闭环已完成；公开仓库边界审计、本地 API 验证和生产配置模板已完成；下一步部署 VPS 并完成远程 API 验证
...
### P1 远程访问与手动 GitHub 同步方案（本地闭环已完成，远程部分暂定）

当前形成以下 P1 方向。VPS 尚未部署，但本地后端与前端同步闭环已经实现并完成真实验证；生产启动配置和最小访问保护模板已经补上，远程部署和 HTTPS 仍未完成：
...
- 已补充生产 `build/start` 脚本、`HOST/PORT` 配置、systemd 服务模板和 Nginx Basic Auth 反向代理模板；模板尚未部署到 VPS。
...
- 已形成并实现 P1 的可配置学习工作区、本地 Git 同步后端和客户端手动同步界面；生产启动配置和访问保护模板已完成，生产 API 地址、VPS 部署和 HTTPS 仍未完成。
- 当前程序仓库最新工作树和公开分支历史不再包含 `sample-library/` 中的真实 Markdown 内容；本地配置已接入私有 `learn-everything` 工作区，首轮边界审计已完成。VPS 部署前仍需在服务器环境中复核 `.env`、Git 凭据和运行时记录的存放位置。
- 公开代码仓库与私有学习资料仓库的内容边界已按当前方案划定：公开仓库保存程序，私有仓库保存真实学习资料；部署时继续保持两者分离。

## 4. 当前结论与边界

静态原型的首轮布局体验已经完成：底部常驻标题会遮挡滚动正文的问题已移除，电脑端顶部栏分隔线也已对齐；接入真实长目录后发现的电脑与 BOOX 窄屏滚动边界问题也已修复。真实 Markdown 已从原始文本变为阅读排版；局域网基本闭环也已经跑通。P0.1 AI 写入安全与恢复已经完成核心实现和真实操作验收。P1 的本地手动同步闭环也已完成并合并到程序仓库 `main`：前端先查看状态，用户点击同步后由后端把允许的 Markdown 修改合并为一个 commit 并 push；中文目录路径也已完成真实验证。公开仓库边界审计、本地只读 API 冒烟验证和生产配置模板也已完成；当前保留现有固定 CSS 阅读预设，下一阶段转向 VPS 部署、HTTPS 和远程 API 验收。
...
### 当前任务：VPS 部署、HTTPS 与远程 API 验收

P0 的“阅读 → 反馈 → OpenAI → 写入下一篇”已经可以使用，P0.1 的写入安全与恢复也已完成核心验收。P1 的本地 Git 同步闭环已经真实跑通并合并到 `main`；公开仓库边界审计、本地只读 API 验证和生产配置模板已经完成，下一步部署 VPS、配置 HTTPS，并让手机/BOOX 访问生产 API。
...
8. 当前下一步：部署 VPS、配置 HTTPS/访问控制，并验证 Nginx → Express → 私有学习库链路。
9. 在关闭个人电脑的情况下，用手机和 BOOX 验收远程阅读、反馈、AI 生成和 GitHub 同步。
10. 最后再评估 Android APK，而不是先把 APK 和远程存储同时引入。
...
- 不规范文章文件名的下一篇编号规则。
- 生成操作记录和快照的保留期限、清理入口。
- P1 远程访问采用一次性访问码、VPN、反向代理鉴权还是其他轻量方案。
- GitHub 远程领先时采用手动拉取、只允许快进，还是增加冲突解决界面。
- Android APK 的具体封装技术和生产环境 API 地址配置。

## 7. 仓库状态提醒

程序仓库的 Git 同步功能和中文路径修复已经合并到 `main`。P0.1 已完成真实生成、强制中断、失败保留、撤销和撤销后重新生成测试；P1 本地同步闭环也已完成真实 GitHub 验证。公开分支历史已完成首轮边界清理；部署 VPS 前仍需确认服务器上的 `.env`、Git 凭据和运行时记录不会进入公开代码仓库或静态发布目录。
