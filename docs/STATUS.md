...
> 更新时间：2026-09-05
>
> 当前阶段：P0/P0.1 已完成；P1 本地学习库与手动 GitHub 同步闭环已完成；VPS 已完成临时 HTTP 部署并验证远程阅读链路；本地已接入并验证阿里云百炼 OpenAI 兼容接口与 Qwen 3.7 Plus；`feat/remember-me-session` 已合并到 `main`，并完成本地登录、退出、记住 7 天和密码更换验证，但尚未部署到 VPS；下一步完成 HTTPS，再决定是否启用
...
这保留了 `dbs-learning` 的反馈驱动节奏：预先写好的路线只是候选，下一次生成仍会重新读取新的反馈并重新判断。当前改动已在本地前后端构建通过，尚未自动推送或部署到 VPS。
...
- 应用登录由 `AUTH_ENABLED` 控制，默认关闭；当前 VPS 仍使用临时 HTTP + Nginx Basic Auth，没有启用这套应用登录。
...
### P1 远程访问与手动 GitHub 同步方案（VPS 临时部署已完成，远程 AI 与 HTTPS 待完成）

当前形成以下 P1 方向。本地后端与前端同步闭环已经实现并完成真实验证；VPS 已完成一轮临时生产部署，Nginx、Basic Auth、systemd、私有学习工作区和远程阅读链路已经验证；本地百炼/Qwen 生成已经完成验证，VPS 生产 AI 与 HTTPS 仍未完成：
...
- 生产环境当前仍使用临时 HTTP + Basic Auth；域名、HTTPS 和 BOOX 的最终公网验收尚未完成。
...
- 应用登录默认由 `AUTH_ENABLED=false` 关闭；生产启用前必须先完成 HTTPS，且当前 Node 单实例会话保存在内存中，服务重启后需要重新登录。
...
- 生产模板目前仍使用 Nginx Basic Auth 保护网页和 API；应用内单用户会话已经合并到 `main` 并完成本地验证，但尚未部署。正式 HTTPS 稳定后，仍需决定保留 Basic Auth 还是迁移到应用登录；API key 仍只保存在服务端，不能下发到手机或 BOOX。
- 已形成并实现 P1 的可配置学习工作区、本地 Git 同步后端和客户端手动同步界面；生产启动配置、访问保护和 VPS 临时部署已完成，生产 HTTPS、远程 AI 生成和 BOOX 最终验收仍未完成。
...
静态原型的首轮布局体验已经完成：底部常驻标题会遮挡滚动正文的问题已移除，电脑端顶部栏分隔线也已对齐；接入真实长目录后发现的电脑与 BOOX 窄屏滚动边界问题也已修复。真实 Markdown 已从原始文本变为阅读排版；局域网基本闭环也已经跑通。P0.1 AI 写入安全与恢复已经完成核心实现和真实操作验收。P1 的本地手动同步闭环也已完成并合并到程序仓库 `main`：前端先查看状态，用户点击同步后由后端把允许的 Markdown 修改合并为一个 commit 并 push；中文目录路径也已完成真实验证。公开仓库边界审计、本地只读 API 冒烟验证和生产配置模板也已完成。当前 VPS 已完成临时 HTTP 部署并验证 Nginx → Express → 私有学习库链路；本地百炼/Qwen AI 适配和真实生成链也已验收。应用内单用户登录已由 PR #8 合并到 `main`，并完成本地验证；下一阶段是完成 HTTPS、测试 VPS 到生产 AI 服务的出站连接，再决定是否把应用登录部署到 VPS。
...
### 当前任务：完成 HTTPS，并决定是否在生产启用应用内“记住登录”会话

P0 的“阅读 → 反馈 → AI → 写入下一篇”在本地已经可以使用，P0.1 的写入安全与恢复也已完成核心验收。P1 的本地 Git 同步闭环已经真实跑通并合并到 `main`；VPS 的临时 HTTP 部署、Basic Auth、systemd 和私有学习库访问已经完成。百炼/Qwen 的本地 API 测试和网页端完整生成测试均已通过。应用内会话功能已经由 PR #8 合并到 `main` 并完成本地验证，但因为生产 Cookie 默认要求 HTTPS，当前先完成 HTTPS，再决定是否将 Basic Auth 迁移到应用登录。
...
10. 当前下一步：测试 VPS 到百炼/Qwen 兼容 API 的出站连接，再更新 VPS 环境变量、构建并重启服务。
11. 配置域名和 HTTPS，并在关闭个人电脑的情况下，用手机和 BOOX 验收远程阅读、反馈、AI 生成和 GitHub 同步。
12. HTTPS 稳定后，选择保留 Nginx Basic Auth 或迁移到已合并的应用登录；若启用应用登录，再配置 `AUTH_ENABLED`、生成 `AUTH_PASSWORD_HASH`，并验证 Cookie 登录、退出、记住 7 天和服务重启后的重新登录行为。
13. 最后再评估 Android APK，而不是先把 APK 和远程存储同时引入。
...
- 不规范文章文件名的下一篇编号规则。
- 生成操作记录和快照的保留期限、清理入口。
- P1 临时远程访问已采用 Nginx Basic Auth；正式 HTTPS 后保留 Basic Auth 还是迁移到已实现的单用户应用登录、是否需要设备级授权，仍待后续决定。
- GitHub 远程领先时采用手动拉取、只允许快进，还是增加冲突解决界面。
- Android APK 的具体封装技术和生产环境 API 地址配置。

## 7. 仓库状态提醒

程序仓库的 Git 同步功能、中文路径修复和 PR #8 应用登录功能已经合并到 `main`。P0.1 已完成真实生成、强制中断、失败保留、撤销和撤销后重新生成测试；P1 本地同步闭环也已完成真实 GitHub 验证。公开分支历史已完成首轮边界清理；VPS 临时部署已完成，应用登录尚未部署。后续维护仍需确认服务器上的 `.env`、Git 凭据和运行时记录不会进入公开代码仓库或静态发布目录。
