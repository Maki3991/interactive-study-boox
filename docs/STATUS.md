...
> 更新时间：2026-08-29
>
> 当前阶段：P0/P0.1 已完成；P1 本地学习库与手动 GitHub 同步闭环已完成；VPS 已完成临时 HTTP 部署并验证远程阅读链路；本地已接入并验证阿里云百炼 OpenAI 兼容接口与 Qwen 3.7 Plus；`feat/remember-me-session` 已完成本地应用会话实现，尚未部署；下一步完成 HTTPS，再决定是否启用
...
- 已新建 `feat/remember-me-session` 功能分支：增加可选的单用户应用登录、7 天记住登录、服务端 scrypt 密码哈希、HttpOnly 会话 Cookie、登录失败限速和统一 API 鉴权；当前只完成本地构建与接口集成验证，未部署到 VPS。
- 应用登录默认由 `AUTH_ENABLED=false` 关闭；生产启用前必须先完成 HTTPS，且当前 Node 单实例会话保存在内存中，服务重启后需要重新登录。
- VPS 直接访问 `api.openai.com` 的 IPv4 HTTPS 请求超时；这属于出站网络问题，不是前端或后端启动问题。国内 AI 服务商适配尚未开始。
...
- 局域网接口此前没有独立的应用内设备授权层；生产模板目前仍使用 Nginx Basic Auth 保护网页和 API，应用内会话已在 `feat/remember-me-session` 分支实现但未部署；API key 仍只保存在服务端，不能下发到手机或 BOOX。
...
静态原型的首轮布局体验已经完成：底部常驻标题会遮挡滚动正文的问题已移除，电脑端顶部栏分隔线也已对齐；接入真实长目录后发现的电脑与 BOOX 窄屏滚动边界问题也已修复。真实 Markdown 已从原始文本变为阅读排版；局域网基本闭环也已经跑通。P0.1 AI 写入安全与恢复已经完成核心实现和真实操作验收。P1 的本地手动同步闭环也已完成并合并到程序仓库 `main`：前端先查看状态，用户点击同步后由后端把允许的 Markdown 修改合并为一个 commit 并 push；中文目录路径也已完成真实验证。公开仓库边界审计、本地只读 API 冒烟验证和生产配置模板也已完成。当前 VPS 已完成临时 HTTP 部署并验证 Nginx → Express → 私有学习库链路；本地百炼/Qwen AI 适配和真实生成链也已验收，下一阶段是把同一配置安全迁移到 VPS，再完成 HTTPS 和远程 AI/API 验收。
...
### 当前任务：完成 HTTPS，并决定是否部署应用内“记住登录”会话

P0 的“阅读 → 反馈 → AI → 写入下一篇”在本地已经可以使用，P0.1 的写入安全与恢复也已完成核心验收。P1 的本地 Git 同步闭环已经真实跑通并合并到 `main`；VPS 的临时 HTTP 部署、Basic Auth、systemd 和私有学习库访问已经完成。百炼/Qwen 的本地 API 测试和网页端完整生成测试均已通过。应用内会话功能已经在独立分支完成本地验证，但因为生产 Cookie 默认要求 HTTPS，当前先完成 HTTPS，再决定是否将 Basic Auth 迁移到应用登录。
...
10. 当前下一步：测试 VPS 到百炼 API 的出站连接，再更新 VPS 环境变量、构建并重启服务。
11. 配置域名和 HTTPS，并在关闭个人电脑的情况下，用手机和 BOOX 验收远程阅读、反馈、AI 生成和 GitHub 同步。
12. HTTPS 稳定后，再评估部署 `feat/remember-me-session`：启用 `AUTH_ENABLED`、生成 `AUTH_PASSWORD_HASH`，并验证 Cookie 登录、退出和服务重启后的重新登录行为。
...
- 不规范文章文件名的下一篇编号规则。
- 生成操作记录和快照的保留期限、清理入口。
- P1 临时远程访问已采用 Nginx Basic Auth；正式 HTTPS、设备级授权和更细粒度访问控制仍待后续决定。
- GitHub 远程领先时采用手动拉取、只允许快进，还是增加冲突解决界面。
- Android APK 的具体封装技术和生产环境 API 地址配置。

## 7. 仓库状态提醒

程序仓库的 Git 同步功能和中文路径修复已经合并到 `main`。P0.1 已完成真实生成、强制中断、失败保留、撤销和撤销后重新生成测试；P1 本地同步闭环也已完成真实 GitHub 验证。公开分支历史已完成首轮边界清理；VPS 临时部署已完成，后续维护仍需确认服务器上的 `.env`、Git 凭据和运行时记录不会进入公开代码仓库或静态发布目录。
